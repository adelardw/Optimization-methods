...
class LogRegL2Oracle(BaseSmoothOracle):
    ...
    def func(self, x):
        ...
        f = np.logaddexp(0, -self.b * self.matvec_Ax(x)).mean() + self.regcoef*np.linalg.norm(x)**2/2
        return f
    ...
    def grad(self, x):
        ...
        res = -self.matvec_ATx(self.b * (scipy.special.expit(-self.b* (self.matvec_Ax(x)))))
        res /= m
    ...
    def hess(self, x):
        ...
        s = -scipy.special.expit(-self.b * self.matvec_Ax(x)) * (scipy.special.expit(-self.b * self.matvec_Ax(x)) - 1)
        hess = self.matmat_ATsA(s)
        hess /= m
        return hess + self.regcoef * np.eye(len(x))

    def hess_vec(self, x, v):
        m = len(self.b) 
        s = -scipy.special.expit(-self.b * self.matvec_Ax(x)) * (scipy.special.expit(-self.b * self.matvec_Ax(x)) - 1)
        h = s * self.b ** 2 * self.matvec_Ax(v) 
        return  1 / m * self.matvec_ATx(h) + self.regcoef * v
...
def create_log_reg_oracle(A, b, regcoef, oracle_type='usual'):
    ...
    def matmat_ATsA(s):
       return scipy.sparse.coo_array(A.T) @ (scipy.sparse.diags(s) @ scipy.sparse.coo_array(A))
...
def hess_vec_finite_diff(func, x, v, eps=1e-5):
    ...
    for i in range(n):
        #for j in range(n):
        Hess_vec[i] = (func(x + eps * orts[i] + eps * v)
                         - func(x + eps * orts[i])
                         - func(x + eps * v)
                         + func(x)) / eps**2
    return Hess_vec